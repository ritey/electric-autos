...
class VehicleDetails
{
    ...
    public function buildCar($car)
    {
        ...
        if ($this->cache->has($key)) {
            ...
            $this->cache->add($key, $data, config('coderstudios.cache_minutes'));
        }
    ...
    public function scrape()
    {
        ...
        foreach ($pages as $type) {
            ...
            $dom = $crawler->filterXPath("//*[contains(@data-gtm-event-action, 'card to vdp')]");

            $ads = $dom->filter('div')->each(function (Crawler $node, $i) {
                $details = [];
                $item = $node->filter('div > h4');
                $details['title'] = '';
                $details['price'] = '';
                $details['link'] = '';
                if (count($item)) {
                ...
                $item = $node->filter('div div:nth-child(1) div div > span._2D69XQ _1llJHC');
                dd($item->text());
            ...
            foreach ($ads as $ad) {
                ...
                if (!empty($ad['title'])) {
                    ...
                    if (8 == $resource['make_id'] && strpos(strtolower(' '.$resource['name']), ' model s ')) {
                        $resource['model_id'] = 11;
                    }
                    if (8 == $resource['make_id'] && strpos(strtolower(' '.$resource['name']), ' model x ')) {
                        $resource['model_id'] = 12;
                    }
                    if (8 == $resource['make_id'] && strpos(strtolower(' '.$resource['name']), ' roadster ')) {
                        $resource['model_id'] = 13;
                    }

                    if (1 == $resource['make_id'] && strpos(strtolower(' '.$resource['name']), ' i3 ')) {
                        $resource['model_id'] = 1;
                    }
                    if (1 == $resource['make_id'] && strpos(strtolower(' '.$resource['name']), ' i8 ')) {
                        $resource['model_id'] = 2;
                    }

                    if (9 == $resource['make_id'] && strpos(strtolower(' '.$resource['name']), ' prius ')) {
                    ...
                    if (11 == $resource['make_id'] && strpos(strtolower(' '.$resource['name']), ' golf ') || strpos(strtolower(' '.$resource['name']), ' e-golf ')) {
